...
BEGIN
  ...
  CALL GetTotalClassesSize(academic_year,semester,faculty,program,module,lecturer,class,@total);
	SET @Query:=CONCAT("SELECT 
        ...
        ROUND((SUM(case when ",question," = '3' then 1 else 0 end)/COUNT(*))*100,2) AS PERCENTAGE_OF_3,
        ROUND((SUM(case when ",question," = '4' then 1 else 0 end)/COUNT(*))*100,2) AS PERCENTAGE_OF_4,
        ROUND((SUM(case when ",question," = '5' then 1 else 0 end)/COUNT(*))*100,2) AS PERCENTAGE_OF_5
