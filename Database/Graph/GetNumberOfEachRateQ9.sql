CREATE DEFINER=`root`@`localhost` PROCEDURE `GetNumberOfEachRateQ9`(academic_year VARCHAR(50), semester VARCHAR(50),
...
BEGIN
	...
	SELECT 
		SUM(case when Q9 = '1' then 1 else 0 end) AS '1',
        SUM(case when Q9 = '2' then 1 else 0 end) AS '2',
        SUM(case when Q9 = '3' then 1 else 0 end) AS '3',
        SUM(case when Q9 = '4' then 1 else 0 end) AS '4',
        SUM(case when Q9 = '5' then 1 else 0 end) AS '5'
	FROM Questionnaire 
