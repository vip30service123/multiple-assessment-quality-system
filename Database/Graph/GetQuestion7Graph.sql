CREATE DEFINER=`root`@`localhost` PROCEDURE `GetQuestion7Graph`(academic_year VARCHAR(50), semester VARCHAR(50),
...
BEGIN
	...
	SELECT 
        ...
        ROUND(AVG(Q7),2) AS MEAN,
        ROUND(stddev_samp(Q7),2) AS SD,
        ROUND((SUM(case when Q7 = '1' then 1 else 0 end)/SUM(Class.size))*100,2) AS PERCENTAGE_OF_1,
        ROUND((SUM(case when Q7 = '2' then 1 else 0 end)/SUM(Class.size))*100,2) AS PERCENTAGE_OF_2,
        ROUND((SUM(case when Q7 = '3' then 1 else 0 end)/SUM(Class.size))*100,2) AS PERCENTAGE_OF_3,
        ROUND((SUM(case when Q7 = '4' then 1 else 0 end)/SUM(Class.size))*100,2) AS PERCENTAGE_OF_4,
        ROUND((SUM(case when Q7 = '5' then 1 else 0 end)/SUM(Class.size))*100,2) AS PERCENTAGE_OF_5
