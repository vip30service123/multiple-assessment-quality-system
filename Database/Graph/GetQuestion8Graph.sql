...
BEGIN
  ...
  CALL GetTotalClassesSize(academic_year,semester,faculty,program,module,lecturer,class,@total);
	SELECT 
        ...
        ROUND((SUM(case when Q8 = '1' then 1 else 0 end)/COUNT(*))*100,2) AS PERCENTAGE_OF_1,
        ROUND((SUM(case when Q8 = '2' then 1 else 0 end)/COUNT(*))*100,2) AS PERCENTAGE_OF_2,
        ROUND((SUM(case when Q8 = '3' then 1 else 0 end)/COUNT(*))*100,2) AS PERCENTAGE_OF_3,
        ROUND((SUM(case when Q8 = '4' then 1 else 0 end)/COUNT(*))*100,2) AS PERCENTAGE_OF_4,
        ROUND((SUM(case when Q8 = '5' then 1 else 0 end)/COUNT(*))*100,2) AS PERCENTAGE_OF_5
